...
- **Latch after pickup until next scope enable:** once a dial locks, pass all further CCs through. Do **not** re-arm mid-session on settle/phys mismatch — that dead-zones Fixed-step discretes (band 0/1) and pause-continue Ultra spins. Fury UI moves rely on feedback dump/auto_outputs to keep the Deck aligned.
